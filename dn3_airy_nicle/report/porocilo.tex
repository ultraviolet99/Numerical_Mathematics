...
\end{equation}
Poiskati želimo čim več njenih ničel (vse ležijo na negativni polosi) na
deset decimalk natančno; vgrajeni reševalci diferencialnih enačb niso
...
$\Omega$ Magnusova vrsta
\begin{equation}
  \Omega(h) = \int_0^h A\,\mathrm d\tau
  + \frac12 \int_0^h\!\!\int_0^{\tau_1} [A(\tau_1), A(\tau_2)]\,
    \mathrm d\tau_2\,\mathrm d\tau_1 + \cdots
\end{equation}
Metoda reda~4 obdrži prva dva člena in ju izračuna z dvotočkovno
Gauss--Legendrovo kvadraturo: z $A_{1,2} = A\bigl(x + (\tfrac12 \mp
\tfrac{\sqrt3}{6})h\bigr)$ je
\begin{equation}
  \sigma = \frac h2 (A_1 + A_2) - \frac{\sqrt3}{12} h^2\, [A_1, A_2],
  \qquad y^+ = e^{\sigma} y,
  \label{eq:magnus}
\end{equation}
z lokalno napako $O(h^5)$. Za naš $A$ je $[A_1, A_2] = (x_2 - x_1)
...
splošni obliki).
Empirični red smo potrdili z meritvijo (levi graf na sliki~\ref{fig:wronskian}).
...
kar da približno $m$ korakov na nihaj. Iz \eqref{eq:wkb} sledi še: ničle
zadoščajo $\tfrac23|a_k|^{3/2} = \tfrac{\pi(4k-1)}{4}$, kar da vodilni
McMahonov člen $|a_k| = \bigl(\tfrac{3\pi(4k-1)}{8}\bigr)^{2/3}$; število
ničel do globine $x$ je $N(x) \approx \tfrac{2}{3\pi}|x|^{3/2}$; odvod v
ničli je $|\mathrm{Ai}'(a_k)| \approx \pi^{-1/2}|a_k|^{1/4}$, zato so ničle
dobro pogojene in pogojenost se z globino še izboljšuje.
...
\begin{figure}[htb]\centering
  ...
  Izmerjena napaka je v praksi skoraj neodvisna od $k$, zakon \eqref{eq:zakon} pa zato precenjuje
  napako pri velikih $k$ in je konzervativna zgornja meja. Po njej natančnost
  pod $10^{-10}$ pri $m=800$ zdrži do $|a_k| \approx 1011$, kar po $N(x)$
  ustreza približno $6800$ ničlam.}
  \label{fig:napaka}
\end{figure}
