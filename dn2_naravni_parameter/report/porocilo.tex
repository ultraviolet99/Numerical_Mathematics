...
\end{equation}
Naloga \cite{skripta} zahteva relativno natančnost pod $5\cdot 10^{-11}$ za \emph{vse}
...
parameter pomeni geometrijsko. Pike na krivulji so razporejene na enakomernih
ločnih razdaljah, torej enakomerno po dolžini krivulje in ne po parametru $t$.
Ustrezni parametri $t$ zato niso enakomerni: tam, kjer je odvod po parametru
majhen (blizu $\tau^2 = 1/9$), se $t$ med sosednjima pikama spremeni za več kot
tam, kjer je odvod velik.
...
$n = 60$ na $[0, t]$. Vozli kvadrature so ničle Legendrovega polinoma $P_n$,
uteži pa so $w_i = 2 / \bigl((1-x_i^2) P_n'(x_i)^2\bigr)$~\cite{gw}. Ničle
poiščemo z Newtonovo metodo, pri čemer vrednosti $P_n$ in $P_{n-1}$ računamo
s tričlensko rekurzivno zvezo
$k P_k(x) = (2k-1)\,x\,P_{k-1}(x) - (k-1)\,P_{k-2}(x)$, odvod pa iz
$P_n'(x) = n\,\bigl(x P_n(x) - P_{n-1}(x)\bigr)/(x^2-1)$. Vozle in uteži
izračunamo enkrat in predpomnimo.


Napako Gauss--Legendrove kvadrature lahko navzgor ocenimo s pomočjo teorije
analitičnih funkcij~\cite{gauss}. Hitrost, s katero napaka pada z redom $n$,
krmilijo kompleksne singularnosti integranda: 
razvejišča $g$ so ničle polinoma $9\tau^4 - 2\tau^2 + 1$, torej
\begin{equation}
...
Za velike $|\tau|$ izpostavimo $g(\tau) = 3\tau^2\sqrt{1 + z}$ z
$z = -\tfrac{2}{9}\tau^{-2} + \tfrac{1}{9}\tau^{-4}$ in koren razvijemo po
binomski vrsti. Vrsta konvergira enakomerno na $[T_0, t]$, zato jo smemo
integrirati po členih, kar da
\begin{align}
...
\end{align}
s konstanto $C = \lim_{t\to\infty}\bigl(s(t) - t^3 + t/3\bigr)$.

Interpolirati funkcijo $s$ na neskončnem intervalu $[T_0, \infty)$ ni mogoče,
zato naredimo inverzijo s substitucijo $u = 1/t$, ki ta interval preslika na
končnega, $u \in (0, 1/T_0]$. Sama $s(1/u)$ pri $u \to 0$ raste čez vse meje,
saj se obnaša kot $u^{-3}$, zato jo pomnožimo z $u^3$ in definiramo
\begin{equation}
  h(u) := u^3\, s(1/u)
        = 1 - \tfrac{u^2}{3} + C u^3 - \tfrac{4}{27}u^4
          - \tfrac{4}{729}u^6 + \cdots, \qquad u \in (0, 1/T_0].
\end{equation}
Desna stran je potenčna vrsta brez pola, zato ima $h$ pri $u = 0$ odpravljivo
singularnost in jo lahko zvezno razširimo z $h(0) = 1$; to je limita
$\lim_{u\to 0} u^3 s(1/u)$, saj vodilni člen $s(t) \sim t^3$ z $u^3$ ravno
okrajšamo. Tako razširjena $h$ je na $[0, 1/T_0]$ gladka in primerna za
polinomsko interpolacijo. Konstante $C$ pri tem ni treba poznati, ker je
vsebovana v interpolantu. Ker je $s(t) = t^3\, h(1/t)$ in $|h| \approx 1$,
relativna napaka interpolanta $h$ neposredno omeji relativno napako $s$.

Funkcijo $h$ interpoliramo s polinomom stopnje $M = 40$ v Čebiševih točkah
druge vrste na $[0, 1/T_0]$. Vrednosti $h(u_j) = u_j^3\, s(1/u_j)$ dobimo
tako, da $s(1/u_j)$ izračunamo s sestavljeno kvadraturo: interval razdelimo na
$[0, T_0]$ in $[T_0, 1/u_j]$ ter na vsakem uporabimo kvadraturo reda $n$.
Enojna kvadratura čez celoten interval pri majhnih $u_j$ ne konvergira, ker so
singularnosti glede na dolžino intervala preblizu izhodišča. Koeficiente nato
izračunamo z diskretno kosinusno transformacijo, vrednotimo pa s Clenshawovim
algoritmom, ki je povratno stabilen z napako $O(M\epsilon\sum|c_k|)$.
...
(slika~\ref{fig:cas}). Lihost obravnavamo z $s(-t) = -s(t)$, pri $t=0$ vrnemo $0$. 
...
\begin{figure}[htb]\centering
  ...
  T_0]$. Razvejišči, ki sta najbližji elipsi, ležita tik ob robu elipse na desni in določata $\rho$.
  Desno: relativna napaka glede na referenco \texttt{mpmath} čez obe veji na 33
...
\begin{figure}[htb]\centering
  ...
  med najpočasnejšim in najhitrejšim klicem je $1{,}20$, kar potrjuje ceno
  $O(1)$.}
  \label{fig:cas}
\end{figure}
