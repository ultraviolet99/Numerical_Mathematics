...
$n = 60$ na $[0, t]$. Vozli kvadrature so ničle Legendrovega polinoma $P_n$. Ničle poiščemo z Newtonovo metodo, pri čemer vrednosti $P_n$ računamo s tročlensko rekurzivno enačbo (referenca na wikipedijo npr. ali drugo literaturo, kjer so opisani Legendrovi polinomi). 
(tričlenska rekurenca), uteži so $w_i = 2 / \bigl((1-x_i^2) P_n'(x_i)^2\bigr)$,
vse pa izračunamo enkrat in predpomnimo.

Napako Gauss-Legendrove kvadrature lahko navzgor ocenimo s pomočjo teorije analitičnih funkcij, kot je opisano v \cite{atap}. Hitrost konvergence krmilijo kompleksne singularnosti integranda: razvejišča
...
točkah druge vrste na $[0, 1/T_0]$; vrednosti $h(u_j)$ dobimo tako, da $s(t)$ izračunamo s sestavljeno
